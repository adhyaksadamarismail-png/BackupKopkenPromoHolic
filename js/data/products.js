/**
 * Products & Menu Data Catalog
 * Integrated with authentic Kopi Kenangan MENU dataset provided by user.
 */
...
export const NO_LARGE = [
  "Fresh Lemonade", 
  "Air Mineral", 
  "Thai Tea", 
  "Babyccino", 
  "Milk Oreo Crumble", 
  "Toffee Nut Choco Macchiato", 
  "Thai Tea Loaded", 
  "Thai Tea Aren", 
  "Thai Tea Coffee", 
...
export const HAS_JUMBO = ["Latte", "Americano"];
...
export const products = rawMenu.map(item => {
  const isFood = item.isFood || item.cat === "Food";
  const noBeans = item.noBeans || isFood;
  const noHot = item.noHot || isFood;
  const isNoLarge = item.noLarge || NO_LARGE.includes(item.name) || isFood;
  
  // Custom option set per item
  const options = {
    suhu: isFood ? [] : (noHot ? [{ id: "ice", name: "Ice", price: 0 }] : defaultOptions.suhu),
    ukuran: isFood ? [] : (isNoLarge ? [{ id: "normal", name: "Normal", price: 0 }] : defaultOptions.ukuran),
    es: isFood ? [] : defaultOptions.es,
    gula: isFood ? [] : defaultOptions.gula,
    beans: (noBeans || isFood) ? [] : defaultOptions.beans,
    syrup: isFood ? [] : defaultOptions.syrup,
    topping: isFood ? [] : defaultOptions.topping
  };
  ...
  return {
    ...
    rawId: item.id,
    name: item.name,
    category: item.cat.toLowerCase(),
    price: item.salePrice,
    originalPrice: item.origPrice,
    image: item.img,
    isBestSeller: BEST_SELLER.includes(item.name),
    noBeans,
    noHot,
    noLarge: isNoLarge,
    isFood,
    oatsideLargeOnly: item.oatsideLargeOnly || false,
    options
  };
});
